...
const watchlist = [
  ...
  { ticker: "TSLA", name: "Tesla", price: "$319.05", change: "âˆ’1.18%", tone: "negative", signal: "Cautious" }
];
    ...
    <main>
      ...
      <section id="top" className="hero">
        ...
        <div className="alert-card" aria-label="Example AI assessment">
          ...
          <div className="evidence"><span>âœ“ 3 source-backed facts</span><a href="#disclaimer">View reasoning â†’</a></div>
          <p className="demo">DEMO EXAMPLE â€” not live market data or financial advice.</p>
        </div>
      </section>
      ...
      <section className="dashboard" aria-label="Dashboard preview"><div className="dashboard-head"><div><p className="eyebrow">YOUR WATCHLIST</p><h2>Today&apos;s signals</h2></div><span className="demo-tag">DEMO DATA</span></div>
        <div className="dashboard-grid"><div className="stocks">{watchlist.map((stock) => <article className="stock" key={stock.ticker}><span className={`status ${stock.tone}`} /><div><strong>{stock.ticker}</strong><small>{stock.name}</small></div><div className="stock-price"><strong>{stock.price}</strong><small className={stock.tone}>{stock.change}</small></div><span className={`pill ${stock.tone}`}>{stock.signal}</span></article>)}</div>
          <article className="latest"><p className="eyebrow">LATEST ASSESSMENT</p><span className="green-label">â— BULLISH</span><h3>AI infrastructure demand remains a key NVIDIA driver.</h3><p>Multiple source-backed developments point to potential positive pressure, with valuation sensitivity as the primary counterargument.</p><div className="latest-footer"><span>Confidence: <b>Medium</b></span><button className="text-button">Explore alert â†’</button></div></article></div>
      </section>
      ...
      <footer id="disclaimer"><a className="brand" href="#top"><span>âŒ</span> MarketPulse <b>AI</b></a><p>MarketPulse AI provides informational and educational market intelligenceâ€”not financial, investment, trading, legal, or tax advice. Assessments can be inaccurate or delayed. Do your own research.</p><small>Â© 2026 MarketPulse AI Â· Delayed quote policy for private beta</small></footer>
    </main>
  );
}
