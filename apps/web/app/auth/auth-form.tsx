...
export default function AuthForm({ mode }: { mode: Mode }) {
  ...
  const [notice, setNotice] = useState<string | null>(null); const [error, setError] = useState<string | null>(null); const [loading, setLoading] = useState(false);
  ...
  return <main className="auth-shell"><Link className="brand" href="/"><span>âŒ</span> MarketPulse <b>AI</b></Link><section className="auth-card"><p className="eyebrow">SECURE ACCOUNT</p><h1>{content.title}</h1><p>{content.subtitle}</p><form onSubmit={submit}>{!onlyPassword && <label>Email address<input type="email" autoComplete="email" required value={email} onChange={(e) => setEmail(e.target.value)} /></label>}{mode !== "forgot" && <label>{mode === "update" ? "New password" : "Password"}<input type="password" autoComplete={mode === "signup" ? "new-password" : "current-password"} required minLength={12} value={password} onChange={(e) => setPassword(e.target.value)} /></label>}{error && <p className="form-message error" role="alert">{error}</p>}{notice && <p className="form-message success" role="status">{notice}</p>}<button className="button" disabled={loading}>{loading ? "Please waitâ€¦" : content.submit}</button></form>{mode === "login" && <><Link href="/forgot-password">Forgot password?</Link><p className="auth-switch">New here? <Link href="/signup">Create an account</Link></p></>}{mode === "signup" && <p className="auth-switch">Already have an account? <Link href="/login">Sign in</Link></p>}</section><p className="auth-disclaimer">Market intelligence onlyâ€”not investment advice. Never share your password with anyone.</p></main>;
}
