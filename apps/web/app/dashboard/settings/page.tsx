...
export default function NotificationSettingsPage() {
  ...
  useEffect(() => { const supabase = createClient(); getMember({ requireOnboarding: true }).then(async (user) => { if (!user) return; setUserId(user.id); const { data: saved, error: loadError } = await supabase.from("notification_preferences").select("in_app_enabled,email_enabled,web_push_enabled,market_hours_only,minimum_severity").eq("user_id", user.id).maybeSingle(); if (loadError) setError("Notification preferences are not ready yet. Please run the notification preferences SQL migration, then refresh this page."); if (saved) setPreferences(saved as Preferences); setReady(true); }).catch(() => window.location.assign("/login")); }, []);
