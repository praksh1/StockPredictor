...
export default function DashboardPage() {
  ...
  async function loadWatchlist() {
    ...
    setCatalog((catalogRows ?? []) as Security[]);
  ...
  return <main className="member-dashboard"><header><Link className="brand" href="/"><span>MP</span> MarketPulse <b>AI</b></Link><button className="text-button" onClick={signOut}>Sign out</button></header><section><p className="eyebrow">YOUR MARKET BRIEF</p><h1>Welcome back.</h1><p className="member-email">Signed in as {email}</p>{error && <p className="dashboard-message error">{error}</p>}{notice && <p className="dashboard-message success">{notice}</p>}<div className="watchlist-layout"><article className="watchlist-card"><div className="card-heading"><div><p className="eyebrow">MY WATCHLIST</p><h2>{items.length} selected stocks</h2></div><span className="demo-badge">NO LIVE PRICES YET</span></div>{items.length ? <div className="watchlist-items">{items.map((item) => <div className="watchlist-item" key={item.id}><div className="ticker-badge">{item.ticker}</div><div><strong>{item.ticker}</strong><span>{item.name} | {item.exchange}</span></div><button aria-label={`Remove ${item.ticker}`} className="remove-button" onClick={() => removeSecurity(item)}>Remove</button></div>)}</div> : <div className="empty-state"><strong>Your watchlist is empty.</strong><span>Use the search panel to add the stocks you follow.</span></div>}</article><aside className="search-card"><p className="eyebrow">ADD A STOCK</p><h2>Build your watchlist</h2><label>Search by ticker or company<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Try NVDA or NVIDIA" /></label><div className="search-results">{suggestions.map((security) => <button key={security.id} onClick={() => addSecurity(security)}><span><strong>{security.ticker}</strong><small>{security.name}</small></span><b>Add</b></button>)}{query && !suggestions.length && <p>No matching stock is available in the beta catalog.</p>}</div><p className="catalog-note">Starter catalog only. Live prices, alerts, and expanded search will be connected through authorized providers in the next data phase.</p></aside></div></section></main>;
}
